...
impl MockObjectController {
    ...
    pub fn expect_TEE_CloseAndDeletePersistentObject1_success_once(
        ...
        self.expect_TEE_CloseAndDeletePersistentObject1()
            .return_once_st(move |object| {
                assert_eq!(object, unsafe { *exp_handle.get() });
                raw::TEE_SUCCESS
            });
    }
    ...
    pub fn expect_TEE_CloseObject_once(&mut self, exp_handle: ValidTestHandle) {
        self.expect_TEE_CloseObject().return_once_st(move |object| {
            assert_eq!(object, unsafe { *exp_handle.get() });
        });
    }
}
...
mod test {
    ...
    fn test_mock_usage() {
        ...
        let result =
            unsafe { raw::TEE_OpenPersistentObject(0, std::ptr::null(), 0, 0, &mut handle) };
