...
impl ObjectHandle {
    ...
    pub fn forget(mut self) {
        self.0 = Box::new(core::ptr::null_mut());
    }
}
...
impl ObjectHandle {
    ...
    pub(crate) fn handle_mut(&mut self) -> &mut raw::TEE_ObjectHandle {
        &mut self.0
    }
...
mod tests {
    ...
    /// Ensures the return value of `handle_mut` can be written.
    ///
    /// TODO: still need a systest to ensure that, I found that if change
    /// `ObjectHandle` from `Object(Box<raw::TEE_ObjectHandle>)` to
    /// `ObjectHandle(raw::TEE_ObjectHandle)`, this unittest still works,
    /// but will result in `Panic by Permission Fault` in TA, so things is kind
    /// of different between unittest and TA.
    #[test]
    fn test_handle_mut() {
        let _lock = SERIAL_TEST_LOCK.lock();
        ...
        let mut obj = ObjectHandle::new_null();
        assert!(obj.is_null());

        let inner_handle = obj.handle_mut();
        *inner_handle = unsafe { *handle.get() };
