...
mod tests {
    ...
    fn test_allocate_and_drop() {
...

        {
            ...
            mock.expect_TEE_AllocateTransientObject().return_once_st(
                move |_, _, obj: *mut raw::TEE_ObjectHandle| {
                    unsafe {
                        *obj = *handle;
                    }
                    raw::TEE_SUCCESS
    ...
    fn test_allocate_fail() {
        ...
        mock.expect_TEE_AllocateTransientObject()
            .return_once_st(|_, _, _| RETURN_CODE);
        set_global_object_mock(mock);
