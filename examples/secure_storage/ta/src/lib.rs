...
pub fn delete_object(paramTypes: uint32_t, params: &mut [TEE_Param; 4]) -> TEE_Result {
    let exp_paramTypes: uint32_t = TEE_PARAM_TYPES(TEE_PARAM_TYPE_MEMREF_INPUT, TEE_PARAM_TYPE_NONE, TEE_PARAM_TYPE_NONE, TEE_PARAM_TYPE_NONE);
    ...
    unsafe {
        ...
        let res =  TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, obj_id, obj_id_sz, TEE_DATA_FLAG_ACCESS_READ | TEE_DATA_FLAG_ACCESS_WRITE_META, &mut object as *mut _);
        if res != TEE_SUCCESS {
...
pub fn create_raw_object(paramTypes: uint32_t, params: &mut [TEE_Param; 4]) -> TEE_Result {
    let exp_paramTypes: uint32_t = TEE_PARAM_TYPES(TEE_PARAM_TYPE_MEMREF_INPUT,TEE_PARAM_TYPE_MEMREF_INPUT,TEE_PARAM_TYPE_NONE,TEE_PARAM_TYPE_NONE);
    if paramTypes != exp_paramTypes {
        return TEE_ERROR_BAD_PARAMETERS;
    }
    ...
    unsafe { 
        ...
        let obj_data_flag: uint32_t = TEE_DATA_FLAG_ACCESS_READ | TEE_DATA_FLAG_ACCESS_WRITE | TEE_DATA_FLAG_ACCESS_WRITE_META | TEE_DATA_FLAG_OVERWRITE;                
        let mut object: TEE_ObjectHandle = ptr::null_mut();
        let mut res: TEE_Result = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, obj_id, obj_id_sz, obj_data_flag, TEE_HANDLE_NULL as TEE_ObjectHandle, ptr::null(), 0, &mut object as *mut _);
        if res != TEE_SUCCESS {
...
pub fn read_raw_object (paramTypes: uint32_t, params: &mut [TEE_Param; 4]) -> TEE_Result {
    let exp_paramTypes: uint32_t = TEE_PARAM_TYPES(TEE_PARAM_TYPE_MEMREF_INPUT,TEE_PARAM_TYPE_MEMREF_OUTPUT,TEE_PARAM_TYPE_NONE,TEE_PARAM_TYPE_NONE);
    if paramTypes != exp_paramTypes {
        return TEE_ERROR_BAD_PARAMETERS;
    }
    ...
    unsafe {
        ...
        let data_sz: uint32_t = params[1].memref.size;
        let mut object: TEE_ObjectHandle = ptr::null_mut();
        let mut res = TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, obj_id, obj_id_sz, TEE_DATA_FLAG_ACCESS_READ | TEE_DATA_FLAG_SHARE_READ, &mut object as *mut _);
        if res != TEE_SUCCESS {
            //EMSG("Failed to open persistent object, res=0x%08x", res);
            TEE_Free(obj_id);
            return res;
        ...
        let mut object_info: TEE_ObjectInfo = TEE_ObjectInfo{objectType: 0, objectSize: 0, maxObjectSize: 0, objectUsage: 0, dataSize: 0, dataPosition: 0, handleFlags: 0};
        ...
        if res != TEE_SUCCESS {
            ...
            if object_info.dataSize > data_sz {
                ...
                res = TEE_ReadObjectData(object, data, object_info.dataSize, &mut read_bytes as *mut _);
...
#[no_mangle]
pub extern "C" fn TA_InvokeCommandEntryPoint(_sessionContext: SessionP, commandID: u32, paramTypes: ParamTypes, params: &mut [TEE_Param; 4]) -> TEE_Result {
    match commandID {
